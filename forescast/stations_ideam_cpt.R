...
e$Date=as.Date(as.character(e$Date),format ="%Y%m%d")

e$all=paste(e2$dpto,"-",e2$station)
...
month1= format(Date[[1]]$Date, "%b") 
year1= format(e$Date, "%Y")
...
rutOrigen="S:\\observed\\weather_station\\col-ideam\\daily-raw\\prec-per-station"
  files <-list.files(rutOrigen,pattern="\\.txt$")
  nom.files<-substring(files,1,nchar(files)-13)
  nom.files=as.numeric(nom.files)
  
idstation=read.csv("catalog_precip.csv",header=T)
attach(idstation)
...
file.copy(paste(rutOrigen,"\\",station_find,"_raw_prec.txt",sep=""),"C:/Users/lllanos/Desktop/Precipitacion/daily/data/")
