...
pronosticos=function(){
  ...
  colnames(Años_org)=Fechas
   ...
   if(length(as.numeric(attributes(na.omit(var_org.na))$na.action))!=0){
    ...
    Años_org2=Años_org[-as.numeric(attributes(na.omit(var_org.na))$na.action),match(names(probabilidades),colnames(Años_org))]
   }else{
  ...
  if(exists("data_d")){
    ...
    Multianual_Libertad1=aggregate(data_d$precip,list(Mes=data_d$month,Año=data_d$year),sum22) #Se carga la precipitación mensual multianual 
    Multianual_Libertad=aggregate(Multianual_Libertad1[,3],list(Mes=Multianual_Libertad1$Mes),mean22)[match(names(probabilidades),colnames(Años_org)),]#Se carga la precipitación mensual multianual 
    ...
    Tmax_Multianual_Libertad1=aggregate(data_d$tmax,list(Mes=data_d$month,Año=data_d$year),mean22) #Se carga la precipitación mensual multianual 
    Tmax_Multianual_Libertad=aggregate(Tmax_Multianual_Libertad1[,3],list(Mes=Tmax_Multianual_Libertad1$Mes),mean22)[match(names(probabilidades),colnames(Años_org)),]#Se carga la precipitación mensual multianual 
    
    tiff("Pronosticos/tmax_pronos.tiff", width = 10, height = 5,units = 'in',res=200)
    r=barplot(as.numeric(Tmax_Multianual_Libertad[,2]),names.arg=names(probabilidades),xpd = FALSE,col="cornsilk1",ylab="Temperatura Máxima (°C)",cex.lab=1,,main="Pronóstico Temperatura Máxima ",cex.main=0.8,ylim=c(min(data_d$tmax,na.rm=T)+2,max(data_d$tmax,na.rm=T)-1))
    ...
    Tmin_Multianual_Libertad1=aggregate(data_d$tmin,list(Mes=data_d$month,Año=data_d$year),mean22) #Se carga la precipitación mensual multianual 
    ...
    r=barplot(as.numeric(Tmin_Multianual_Libertad[,2]),names.arg=names(probabilidades),ylim=c(min(data_d$tmin,na.rm=T)+2,max(data_d$tmin,na.rm=T)+3),xpd = FALSE,col="cornsilk1",ylab="Temperatura Mínima(°C)",cex.lab=1,main="Pronóstico Temperatura Mínima ",cex.main=0.8)
